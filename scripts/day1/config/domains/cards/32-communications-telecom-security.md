...
**Method:** Build for confidentiality by construction: assume every link is observed and design so observation yields nothing usable. Review a channel by what an observer learns from timing, size and metadata even when the payload is sealed, then close the gap. Where a protocol cannot meet the bar, say so and name the replacement, and check post-quantum readiness (ML-KEM, ML-DSA) of the estate's key exchange.
